...
export default class Streams extends Component {
    ...
    componentDidMount() {
      this.getStreams(this.state.ksqlServerId, this.state.clusterName);
    }
    ...
    handleDeletes = (record) => {
      ...
      this.setState(
        ...
        () => {
          axios
            .delete(`/ksql/drop_stream?ksqlServerId=${serverId}&clusterName=${clusterName}&streamName=${record.name}`)
            .then((response) => {
              if (response.data.code === 200) {
                this.getStreams(serverId, clusterName);
    ...
    getStreams = (id, name) => {
      this.setState(
        ...
        () => {
          axios.post(`/ksql/show_streams?ksqlServerId=${id}&clusterName=${name}`).then((response) => {
            if (response.data.code === 200) {
      ...
      return (
        <div>
          ...
          <Table
            ...
            hasBorder={false}
          >
            <Table.Column title="Stream" dataIndex="name" />
