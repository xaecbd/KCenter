...
export default class ConsumerGroup extends Component {
  ...
  handelGroupChart = (record) => {
    ...
    this.props.history.push(`/monitor/consumer/topic/consumer_offsets/chart/${this.props.match.params.clusterID}/${record.topic}/${this.props.match.params.consummerGroup}/${zkName}`);
  };
  ...
  handelDialog = (record) => {
    ...
    obj.consummerGroup = this.props.groupName;
  ...
  render() {
    ...
    const render = (value, index, record) => {
      ...
      return <div>{type}&nbsp;<a style={styles.topicLink} onClick={() => this.handelGroupChart(record)}>{value}</a></div>;
    };
    const renderYellowObj = (value) => {
        if (value === '') {
          if (this.props.config.isZk) {
            return this.renderIceImag(warning, 'SimpleConsumerGroup无法判断其状态');
        }
        return this.renderIceImag(warning, 'SimpleConsumerGroup无法判断其状态');
        }
        return this.renderIceImag(warning, 'SimpleConsumerGroup无法判断其状态');
    ...
    return (
      <div className={this.props.config.isZk ? 'zktest' : ''}>
        ...
        <Table className="custom-table" dataSource={data} cellProps={getCellProps}>
          <Table.Column title="Topic" dataIndex="topic" cell={render} width={190} />
          ...
          {
              this.props.config.isZk ? <Table.Column title="Last Seen" dataIndex="clientId" cell={splitTimer} width={90} /> : null }
